...
const  Block: NextPage<BlockProps> = ({block}) => {
    ...
    if(block.element === "input"){
        return (
            <div className={`flex text-white items-center py-1 ${block.checked? "line-through opacity-70": ""}`}>
    ...
    if(block.element === "img"){
        return (
            <img src={block.img?.url} className={`max-w-[100%] max-h-96 w-auto object-cover mx-auto`} />
        )
    }
        ...
        if(block.element === "ol"){
            ...
            return (
                <div className={`text-white py-1 ${styleMap[block.element]}`}>
                    {index}.&nbsp;&nbsp;<TextBlock text={block.text!}/>
                </div>
            )
